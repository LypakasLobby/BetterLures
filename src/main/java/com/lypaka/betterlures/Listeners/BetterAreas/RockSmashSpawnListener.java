...
import com.lypaka.betterareas.API.AreaRockSmashSpawnEvent;
...
import com.lypaka.betterareas.Areas.Spawns.RockSmashSpawn;
...
public class RockSmashSpawnListener {

    @SubscribeEvent
    public void onRockSmash (AreaRockSmashSpawnEvent event) {
        ...
        if (LurePokemonList.lurePokemonMap.containsKey(player.getUniqueID())) {
            ...
            List<Pokemon> possiblePokemon = getHeadbuttablePokemonFromArea(spawns.getRockSmashSpawns());
            ...
            for (Map.Entry<Lure, Map<Pokemon, Double>> entry : pokemonMap.entrySet()) {
                ...
                if (RandomHelper.getRandomChance(lure.getSpawnChance())) {
                    ...
                    int level = getLevelForPokemon(spawns.getRockSmashSpawns(), time, weather, selectedSpawn);
    ...
    private static int getLevelForPokemon (List<RockSmashSpawn> spawns, String time, String weather, Pokemon pokemon) {
        ...
        for (RockSmashSpawn spawn : spawns) {
    ...
    private static List<Pokemon> getHeadbuttablePokemonFromArea (List<RockSmashSpawn> spawns) {
